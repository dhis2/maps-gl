...
If the change touches `src/index.js`'s exported surface (`Map`, `layerTypes`,
`controlTypes`, `loadEarthEngineWorker`, `poleOfInaccessibility`), say so explicitly —
that's a semver-relevant, cross-repo-consumer-affecting change (see the
`maps-gl-architecture` skill), not an ordinary internal edit.
