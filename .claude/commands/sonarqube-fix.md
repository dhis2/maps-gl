...
**Note**: this project (`dhis2_maps-gl` on SonarCloud) is public, so every call below works anonymously — no `SONAR_TOKEN` needed. There is no local scanner/CI step to run — analysis happens automatically server-side on every push (SonarCloud "Automatic Analysis"), so there's no local "publish" step, just push and wait.
...
```bash
curl -s "https://sonarcloud.io/api/issues/search?componentKeys=dhis2_maps-gl&pullRequest=<pr-number>&resolved=false&ps=100" \
